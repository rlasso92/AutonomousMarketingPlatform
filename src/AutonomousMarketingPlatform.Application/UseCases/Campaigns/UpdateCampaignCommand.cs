...
/// </summary>
public class UpdateCampaignCommand : IRequest<CampaignDetailDto>
{
    public Guid TenantId { get; set; }
    public Guid UserId { get; set; }
    public Guid CampaignId { get; set; }
    public UpdateCampaignDto Campaign { get; set; } = null!;
}

/// <summary>
/// Handler para actualizar campaña.
/// </summary>
public class UpdateCampaignCommandHandler : IRequestHandler<UpdateCampaignCommand, CampaignDetailDto>
{
    ...
    private readonly IValidator<UpdateCampaignDto> _validator;
    ...
    public UpdateCampaignCommandHandler(
        ...
        IValidator<UpdateCampaignDto> validator,
    ...
    public async Task<CampaignDetailDto> Handle(UpdateCampaignCommand request, CancellationToken cancellationToken)
    {
        ...
        campaign.Description = request.Campaign.Description;
        campaign.Status = request.Campaign.Status;
        campaign.StartDate = request.Campaign.StartDate;
        campaign.EndDate = request.Campaign.EndDate;
        campaign.Budget = request.Campaign.Budget;
        campaign.Objectives = request.Campaign.Objectives != null
            ? JsonSerializer.Serialize(request.Campaign.Objectives)
            : null;
        campaign.TargetAudience = request.Campaign.TargetAudience != null
            ? JsonSerializer.Serialize(request.Campaign.TargetAudience)
            : null;
        campaign.TargetChannels = request.Campaign.TargetChannels != null
            ? JsonSerializer.Serialize(request.Campaign.TargetChannels)
            : null;
        campaign.Notes = request.Campaign.Notes;
        campaign.UpdatedAt = DateTime.UtcNow;

        await _campaignRepository.UpdateAsync(campaign, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Campaña actualizada: {CampaignId} por usuario {UserId} en tenant {TenantId}",
    ...
    private CampaignDetailDto MapToDetailDto(Domain.Entities.Campaign campaign)
    {
